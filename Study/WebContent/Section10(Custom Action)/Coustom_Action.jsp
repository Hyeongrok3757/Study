...
<%
	...
	// ex) 	1. test폴더 생성 -->  test폴더안에 WEB-INF폴더를 만들고 클래스 파일을 제외한 나머지 파일들을 저장(TLD파일 등) -> 태그 클래스의 컴파일 결과물들을 
	//			test폴더 아래에 저장.(웹애플리케이션의 WEB-INF/classes폴더 아래에 있는 tool 폴더를 전체로 복사해서 넣는다.)
	//		2. 태그 클래스 등록시 TLD파일, web.xml 파일을 이용했는데 만들 태그 라이브러리에는 web.xml이 들어가지 않는다. TLD파일의 URI를 TLD파일 안에 직접 써 넣어야 함.
	//			--> TLD파일안에 <short-name>엘리먼트 다음위치에 <uri>엘리먼트를 추가하고 본체에 TLD파일의 URI를 써 넣는다.
	//		3. JDK의 bin폴더에 있는 jar.exe 프로그램을 이용해서 만든다.
	//			--> jar cvf0 tool.jar *	
	//					=> cvf0 : 옵션(c:생성, v:들어가는 파일들의목록 보기, f:옵션다음에 생성할 JAR파일의 이름이 온다는 의미, 0:ZIP압축 알고리즘을 사용하지 말라는 의미)
	//					=> tool.jar : 생성할 JAR파일 이름
	//					=> * : JAR파일에 들어갈 파일들
	// 생성된 태그 라이브러리(jar)는 웹어플리케이션의 WEB-INF/lib 폴더에 넣고 씀.
	
	// ★ 태그 파일을 모아서 태그 라이브러리를 만드는 방법
	// 1. 디렉터리 계층 구조를 만들고 파일들을 그곳에 저장
	// 2. TLD 파일을 만들기
	// 3. 디렉터리 계층 구조 전체를 JAR 파일로 만듬
	// ex) 1. test폴더를 생성 --> test폴더안에 META-INF폴더를 만들고 태그파일을 META-INF폴더 아래의 tags폴더나 그 아래의 서브폴더에 저장한다.
	//																	( 웹애플리케이션의 WEB-INF아래에 있는 tags폴더를 통째로 복사해서 저장)
	//	TLD파일 작성
	//	<taglib xmlns="http://java.sun.com/xml/ns/javaee" version="2.1">
	//		<tlib-version>1.0</tlib-version>
	...
	//		</tag-file>
	//	</taglib>
	//	생성한 TLD파일을 test/META-INF폴더 안에 넣는다.
	//	태그 라이브러리 생성 --> jar cvf0 tool.jar *
